				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="row">
							...
							<?php
$uid=$_SESSION['login'];
				$stmt=$mysqli->prepare("SELECT email FROM stdfill WHERE email=? ");
				...
				if($rs)
				{ ?>
			<h3 style="color: red" align="left">Your elective already received</h3>
				<?php }
				else{
							echo "";
							}			
...
<label class="col-sm-4 control-label"><h4 style="color: green" align="left">Electiv related information </h4> </label>
</div>
<div class="form-group">
...
<?php 
$query ="DELETE FROM sub2 WHERE subname IN ( SELECT ele2 FROM stdfill GROUP BY ele2 HAVING COUNT(*) = 25 )";
$stmt2 = $mysqli->prepare($query);
$stmt2->execute();
$res=$stmt2->get_result();
?>
...
<span id="" style="font-size:12px;"></span>
	...
	<?php	
$aid=$_SESSION['id'];
	$ret="select * from stdreg where id=?";
...
<div class="col-sm-8">
<input type="text" name="usn" id="usn"  class="form-control" value="<?php echo $row->usn;?>" readonly>
</div>
</div>
...
<div class="col-sm-8">
<input type="text" name="sem" class="form-control"value="<?php echo $row->sem;?>" readonly >
</div>
</div>
...
<div class="col-sm-8">
<input type="text" name="contact" id="contact"  class="form-control" value="<?php echo $row->contact;?>" readonly>
</div>
</div>
